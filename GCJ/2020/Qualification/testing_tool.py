  ...
  def _ParseContestantInput(self, response):
    ...
    try:
      ...
      if not 1 <= num <= self.len:
        return None, _ERROR_MSG_INPUT_OUT_OF_RANGE + f'### {num}'
      return num, None
    except ValueError:
      return None, _ERROR_MSG_INVALID_INPUT + f'### {response} ### {self.arrs}'
      ...
      if isinstance(contestant_input, str):
        if self.arr != contestant_input:
          return _ERROR_MSG_WRONG_ANSWER_FORMAT_STR.format(
              contestant_input[:2 * self.len], self.arr) + f'{self.arrs}'
