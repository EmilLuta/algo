...
fn main() {
    ...
    let mut boards_left: HashSet<usize> = (0..input.boards.len()).collect();
    let mut last_win_val = -1;
    let mut last_win_index = 0;
    for i in 0..input.arr.len() {
        let val = input.arr[i];
        let mut new_boards_left: HashSet<usize> = boards_left.clone();
        for j in &boards_left {
            let winner = input.boards[*j].mark(val);
            if winner {
                new_boards_left.remove(j);
                last_win_val = val;
                last_win_index = *j;
            }
        }
        boards_left = new_boards_left;
        if boards_left.len() == 0 {
            break;
        }
    }
    println!(
        "answer = {}, with last_win_index = {} and last_win_val = {}",
        input.boards[last_win_index].get_sum_left() * last_win_val,
        last_win_index,
        last_win_val
    );
}
